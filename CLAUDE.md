...
> **Tabela canónica de agentes**: `obsidian_vault/AGENTS_REGISTRY.md` — quem é
> quem (handles `area.funcao`), o que faz, ficheiro fonte e modus operandi.
> Estende `config/agents.yaml` com renomes humanos e cobre também perpetuums
> e specialists. Consultar primeiro antes de criar agente novo.
...
5. **Spec-first para UI** *(Phase MM, 2026-05-08)* — qualquer componente .tsx novo de UI requer `obsidian_vault/specs/<component>.spec.md` committed **antes** do código. Spec deve cobrir: purpose, hierarchy intent (hero/primary/secondary/tertiary), density tradeoffs, anti-patterns, success metrics. Adoptado de `KAOPU-XiaoPu/web-design` (301⭐) após critique 2026-05-08 *"AI slop"* sobre Phase LL Sprint 3 components. Ver [[obsidian_vault/skills/Mission_Control_Design_Roadmap_v2]] para skills tier-1 (`impeccable` / `open-design` / `taste-skill`) que enforçam taste antes de merge.
...
| **Library ingest books** (PDF→chunks) | `python -m library.ingest [--engine pypdf\|markitdown\|auto] [--force]` — markitdown preserva tabelas/headers (melhor RAG); default pypdf p/ backwards compat |
| **Universal extractor** (PDF/DOCX/XLSX/PPTX/HTML/IMG/AUDIO→MD) | `from library._md_extract import extract_text; extract_text(path, engine='markitdown')` — MS markitdown 1ª, pdfplumber/pypdf fallback. Phase MCP-5 (2026-05-10) |
| **CVM PDF extractor (markitdown opt)** | `python monitors/cvm_pdf_extractor.py --engine markitdown` — preserva structure de fatos relevantes p/ Qwen extraction |
| **Portal scraper** (Playwright + markitdown, public sites) | `python fetchers/portal_playwright.py <url> [--md] [--screenshot] [--ttl HOURS] [--selector CSS]` — JS-rendered RI/B3/fiis/SPA pages → HTML/MD/PNG cached em `data/portal_cache/` |
...
| **Exa neural search** (news monitoring · find-similar · deep research) | `python fetchers/exa_fetcher.py --news TK --name "Nome" \| --similar URL \| --contents URL \| "query" --deep` — `EXA_API_KEY` em `.env` (plugin `exa` no Claude Code usa OAuth, sep). Funções: `search_news/find_similar/deep_research/get_contents`. Complemento ao Tavily p/ recall semântico. Na cascade como kind `web_news` (`ii fetch us web_news TK`); `exa_news_monitor.py` corre em batch no `daily_run.bat`. |
...
| **Agent call (role-based)** | `python -m agents._agent <role> --input '{"...": "..."}' [--ticker X] [--market br\|us]` — role ∈ {classification, extraction, research, synthesis, critic, decision}. 3B default para classification, 14B para extraction/research/synthesis/critic, 32B para decision. Cache de 1h via `data/agent_decisions.db`; retry + escalation automáticos. Configurado em `config/agents_governance.yaml`. |
| **Agent decisions stats** | `ii agent-stats` — agg 7d de agent_decisions.db (success rate, avg attempts, escalations por role/model) |
| **Overnight backfill** | `ii overnight [--market br\|us\|both] [--dry-run] [--skip-fetch] [--skip-strategies]` — pre-warm: prices+fundamentals+macro via cache; corre 5 strategy engines × universe; persiste em `strategy_runs` table; salva `Allocation_<MKT>_<DATE>.json` no vault; relatório md em `obsidian_vault/Bibliotheca/Overnight_Backfill_<DATE>.md`; idempotente. |
...
| **Exa news monitor** (varre universo → tabela `news`) | `python scripts/exa_news_monitor.py [--holdings] [--market br\|us\|both] [--num N] [--max-age-hours H] [--limit N] [--dry-run] [--md]` — 1 chamada Exa/ticker; persiste `INSERT OR IGNORE` (source `exa:<domain>`); `--md` escreve digest em `obsidian_vault/Bibliotheca/Exa_News_<date>.md`. Overnight ao lado de `ii news`. |
...
| **`ii deepdive <TK>`** (V10 Personal Equity Valuator) | Pipeline 4-camadas: Auditor (Piotroski+Altman+Beneish+**Moat**) ‖ Scout (yfinance: news/insider/short/consensus) → Historian delta → Strategist Llama dossier 5k palavras. `--no-llm` salta strategist. `--save-obsidian` escreve `obsidian_vault/dossiers/<TK>.md`. JSON sempre em `reports/deepdive/`. |
...
| **Moat Score** (durabilidade competitiva 0-10) | `python -m scoring.moat <TK> [--market br\|us] [--json]` — 4 sub-scores: pricing_power (GM level+stability), capital_efficiency (ROIC level+persistence), reinvestment_runway (rev CAGR + FCF/NI), scale_durability (op margin trend + buybacks). Overall ≥7 STRONG, 5-7 NEUTRAL, <5 WEAK. Exclui Banks/Holding/REITs/FIIs. Requer ≥3 anos `deep_fundamentals`. Wired no `ii deepdive` Auditor layer. |
| **Fair Value Forward** (quality-aware, EXPERIMENTAL) | `python -m analytics.fair_value_forward [--ticker X\|--all-us] [--no-analyst]` — DCF 2-estágios sobre owner earnings (base = forward EPS consenso ou mediana NI 3y × conversão de caixa; g cortado por sector; fade→2,5-3%; desconto 8,5-9,5%; banda de sanidade 16-25× OE). Bancos: EPS_norm ×13-14. REITs: AFFO×P/AFFO (`_AFFO_OVERRIDE` hand-maintained). **NÃO está no engine/daily_run/ii** — leitura lado-a-lado do `our_fair` conservador (Opção B do plano de fusão, ver `obsidian_vault/Sessions/FairValue_Forward_Audit_2026-05-11.md`). Output `obsidian_vault/Bibliotheca/FairValue_Forward_<DATE>.md`. Correr com `.venv` p/ yfinance. |
