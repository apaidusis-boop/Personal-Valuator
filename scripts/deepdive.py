...
Output:
  - JSON em reports/deepdive/<TICKER>_<YYYYMMDD_HHMM>.json (source of truth)
  - Section inserida em obsidian_vault/hubs/<TICKER>.md  (--save-obsidian)
    (replaces today's section if re-run; otherwise inserts at top of year)
...
HUBS_DIR = ROOT / "obsidian_vault" / "hubs"
...
def _build_deepdive_section(ticker: str, output: dict, today_iso: str) -> str:
    """Render the deepdive content as a markdown section to embed in a hub.

    Section starts with `#### <today> · Deepdive (V10 4-layer)` — matches the
    chronological-journal pattern used by build_merged_hubs.py.
    """
    audit = output["audit"]
    scout = output["scout"]
    meta = scout.get("meta") or {}
    ...
    delta_text = output.get("delta") or "(sem delta vs análise anterior)"
    dossier_text = output.get("dossier") or "*(dossier não gerado — flag --no-llm ou erro LLM)*"

    lines = [
        f"#### {today_iso} · Deepdive (V10 4-layer)",
        f"_generated by `ii deepdive {ticker}` em {datetime.now().isoformat(timespec='minutes')}_",
        "",
...
def _save_obsidian(ticker: str, output: dict) -> Path:
    """Insert (or replace) today's Deepdive section into obsidian_vault/hubs/<TK>.md.

    - If hub doesn't exist: write a minimal scaffold seeded with this deepdive.
      Next run of `scripts/build_merged_hubs.py` will enrich it with historical
      content from filings/overnight/etc.
    - If today's `#### <YYYY-MM-DD> · Deepdive` section already exists: replace it.
    - Otherwise: insert at top of this year's section in the Histórico, creating
      the year subsection if missing.
    """
    HUBS_DIR.mkdir(parents=True, exist_ok=True)
    hub_path = HUBS_DIR / f"{ticker}.md"
    today_iso = date.today().isoformat()
    year = today_iso[:4]
    section = _build_deepdive_section(ticker, output, today_iso)
    ...
    if not hub_path.exists():
        scaffold = "\n".join([
            ...
            "tags: [hub, ticker, deepdive_seeded]",
            'parent: "[[_TICKERS_INDEX]]"',
            "---",
            "",
            f"# {ticker}",
            "",
            "> Hub seeded por `ii deepdive`. Run `python scripts/build_merged_hubs.py` para enriquecer com filings/overnight/wiki/etc.",
            "",
            "## 🎯 Hoje",
            f"- Último deepdive: {today_iso}",
            "",
            ...
            "",
            "```bash",
            f"ii panorama {ticker} --write",
            f"ii deepdive {ticker} --save-obsidian",
            f"ii verdict {ticker} --narrate --write",
            f"ii fv {ticker}",
            "```",
            "",
        ])
        hub_path.write_text(scaffold, encoding="utf-8")
        return hub_path

    text = hub_path.read_text(encoding="utf-8")
    today_marker = f"#### {today_iso} · Deepdive"
    h4_re = re.compile(r"^#### ", re.MULTILINE)
    h2_re = re.compile(r"^## ", re.MULTILINE)
    h3_re = re.compile(r"^### ", re.MULTILINE)

    if today_marker in text:
        # Replace from today_marker to the next #### / ### / ## (whichever comes first)
        start = text.index(today_marker)
        cursor = start + len(today_marker)
        candidates = []
        for rx in (h4_re, h3_re, h2_re):
        ...
        year_marker = f"### {year}\n"
        if year_marker in text:
            idx = text.index(year_marker) + len(year_marker)
            text = text[:idx] + "\n" + section + "\n" + text[idx:]
        else:
            # Find Historical journal header and prepend a fresh year section there
            journal_re = re.compile(r"^## .*Hist[oó]rico", re.MULTILINE)
            jm = journal_re.search(text)
            if jm:
                # Insert after the first blank line following the header
                tail = text[jm.end():]
                blank = re.search(r"\n\n", tail)
                if blank:
                    insert_at = jm.end() + blank.end()
                    text = (
                        text[:insert_at]
                        + f"### {year}\n\n"
                        + section
...
def main() -> int:
    ...
    ap.add_argument("--save-obsidian", action="store_true",
                    help="insert/replace today's section in obsidian_vault/hubs/<TICKER>.md")
