...
echo. >> "%LOG%"
echo [CONTENT-TRIGGER] auto_verdict_on_content.py  ^(content vs verdict mismatch^) >> "%LOG%"
"%PY%" scripts\auto_verdict_on_content.py --hours 24 --quiet >> "%LOG%" 2>&1
echo CONTENT-TRIGGER exit code: %errorlevel% >> "%LOG%"
...
echo. >> "%LOG%"
echo [YT-POLL] yt_poll.py  ^(channels morning calls + research, retry x3^) >> "%LOG%"
"%PY%" scripts\_retry.py --tag YT-POLL --attempts 3 --backoff 60,300,900 --timeout 1800 -- "%PY%" scripts\yt_poll.py --max-per-channel 2 >> "%LOG%" 2>&1
echo YT-POLL exit code: %errorlevel% >> "%LOG%"

echo. >> "%LOG%"
echo [POD-POLL] pod_poll.py  ^(WSJ/FT/Bloomberg morning briefings, retry x3^) >> "%LOG%"
"%PY%" scripts\_retry.py --tag POD-POLL --attempts 3 --backoff 60,300,900 --timeout 1800 -- "%PY%" scripts\pod_poll.py --max-per-feed 1 >> "%LOG%" 2>&1
echo POD-POLL exit code: %errorlevel% >> "%LOG%"
