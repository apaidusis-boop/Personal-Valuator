...
def download_ipe(year: int) -> list[dict]:
    """Pre-checks circuit breaker; records ok/fail back to it on outcome."""
    try:
        from agents._health import _is_tripped, record as _hb_record
        tripped, why = _is_tripped("cvm")
        if tripped:
            raise RuntimeError(f"cvm circuit breaker tripped: {why}")
    except ImportError:
        _hb_record = None  # health module not available — proceed without breaker

    url = CVM_URL.format(year=year)
    sess = _session_with_retries()
    try:
        r = sess.get(url, timeout=60)  # was 180; faster fail for circuit breaker
        r.raise_for_status()
        if _hb_record:
            _hb_record("cvm", "ok")
    except Exception as e:
        if _hb_record:
            _hb_record("cvm", "fail", f"{type(e).__name__}: {str(e)[:200]}")
        raise
...
def run_all(year: int | None = None, only: str | None = None) -> dict:
    ...
    try:
        ...
        # Circuit breaker tripped — graceful skip, no stack trace. Auto-reset
        # via agents._health (6h cooldown). Cron logs just see clean message.
        if "circuit breaker" in str(e).lower():
            _log({"event": "cvm_skipped_breaker_tripped", "year": year, "reason": str(e)})
            return {}
        raise
