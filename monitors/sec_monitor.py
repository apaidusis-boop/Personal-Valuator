...
def _hb():
    """Lazy import of health breaker — None if unavailable."""
    try:
        from agents._health import _is_tripped, record
        return _is_tripped, record
    except ImportError:
        return None, None
...
def fetch_ticker_map(sess: requests.Session) -> dict[str, str]:
    ...
    if cache.exists() and (time.time() - cache.stat().st_mtime) < 7 * 86400:
        ...
        is_tripped, record = _hb()
        if is_tripped:
            tripped, why = is_tripped("sec")
            if tripped:
                raise RuntimeError(f"sec circuit breaker tripped: {why}")
        ...
        try:
            ...
            if record:
                record("sec", "fail", f"{type(e).__name__}: {str(e)[:200]}")
            raise
