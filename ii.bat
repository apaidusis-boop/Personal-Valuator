...
if /i "%CMD%"=="synthesis" ("%PY%" -X utf8 -m agents.daily_synthesizer %ARGS%) & goto :EOF
if /i "%CMD%"=="ticker-insights" ("%PY%" -X utf8 "%ROOT%scripts\inject_ticker_insights.py" %ARGS%) & goto :EOF
if /i "%CMD%"=="content-trigger" ("%PY%" -X utf8 "%ROOT%scripts\auto_verdict_on_content.py" %ARGS%) & goto :EOF
...
if /i "%CMD%"=="budget" ("%PY%" -X utf8 -m agents._budget %ARGS%) & goto :EOF
