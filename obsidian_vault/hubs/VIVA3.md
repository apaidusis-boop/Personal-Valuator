---
type: ticker_hub
ticker: VIVA3
market: br
sector: Consumer Disc.
currency: BRL
bucket: research_pool
is_holding: false
generated: 2026-05-14
sources_merged: 3
tags: [hub, ticker, merged]
parent: "[[_TICKERS_INDEX]]"
---

# VIVA3 — Vivara

> **Hub mergeado**. Todo o conteúdo per-ticker do vault foi absorvido aqui (panorama, dossier, story, council, IC debate, variant, RI, filings, overnights, drips, wiki, reviews por persona, sessions). Ficheiros-fonte estão no `cemetery/2026-05-14/`.

`sector: Consumer Disc.` · `market: BR` · `currency: BRL` · `bucket: research_pool` · `3 sources merged`

## 🎯 Hoje

- **Fundamentals** (2026-05-13): P/E 9.08 · P/B 1.80 · DY 3.1% · ROE 20.7% · ND/EBITDA 1.12 · Dividend streak 7

## 📜 Histórico (conteúdo absorvido, ordem cronológica desc)

> Todas as fontes consolidadas (vault + JSON deepdives). Cada bloco mantém o título original e foi rebaixado 3 níveis (h1→h4) para encaixar.


### 2026

#### 2026-05-13 · Overnight scrape
_source: `cemetery\2026-05-14\ABSORBED-overnight-per-ticker\Overnight_2026-05-13\VIVA3.md` (cemetery archive)_

#### VIVA3 — Pilot Deep Dive (2026-05-12)

- **Market**: BR
- **Sector**: Consumer Disc.
- **RI URLs scraped** (1):
  - https://ri.vivara.com.br/
- **Pilot rationale**: known (watchlist)

##### Antes (estado da DB)

**Posição activa**: (nenhuma — watchlist ou holding sem qty)

- Total events na DB: **0**
- deep_fundamentals (rows anuais): **0**
- Última cotação DB: 2026-05-11 → close=24.0
- Último fundamentals snapshot: period_end=2026-05-11 · ROE=0.20746 · DY=0.029069125 · P/E=9.638555
- Score (último run): score=0.8 · passes_screen=0

**Últimos 5 events em DB**:

_(zero events em DB)_

##### Agora (RI scrape live)

- Scrape: ❌ FALHOU — Traceback (most recent call last):
  File "C:\Users\paidu\investment-intelligence\fetchers\portal_playwright.py", line 233, in <module>
    main()
    ~~~~^^
  File "C:\Users\paidu\investment-intelligence\fetchers\portal_playwright.py", line 220, in main
    result = fetch(
        args.url,
    ...<5 lines>...
        headless=not args.no_headless,
    )
  File "C:\Users\paidu\investment-intellig


### (undated)

#### — · Panorama
_source: `cemetery\2026-05-14\ABSORBED-tickers\tickers\VIVA3.md` (cemetery archive)_

#### VIVA3 — VIVA3

#watchlist #br #consumer_disc.

##### Links

- Sector: [[sectors/Consumer_Disc.|Consumer Disc.]]
- Market: [[markets/BR|BR]]
- Peers: [[GRND3]] · [[PLPL3]] · [[SEER3]]

##### Snapshot

- **Preço**: R$27.76  (2026-05-07)    _-1.80% 1d_
- **Screen**: 0.6  ✗ fail
- **Altman Z**: n/a ()
- **Piotroski**: None/9
- **Div Safety**: 78.0/100 (WATCH)

##### Fundamentals

- P/E: 10.555133 | P/B: 2.2123046 | DY: 2.51%
- ROE: 22.75% | EPS: 2.63 | BVPS: 12.548
- Streak div: 7y | Aristocrat: None

##### Dividendos recentes

- 2025-12-22: R$0.6977
- 2025-04-23: R$0.6602
- 2024-04-30: R$0.3729
- 2023-04-28: R$0.3642
- 2022-05-02: R$0.0539

##### 🎙️ Recent insights & mentions

_Auto-gerado · 2026-05-13 20:46 UTC · yt=0 · analyst=1 · themes=0_

###### 📰 Analyst reports (últimos 120d)

| Data | Fonte | Kind | Stance | PT | Claim |
|---|---|---|---|---:|---|
| 2026-04-24 | SUNO | rating | neutral | 25.00 | [Suno Dividendos] VIVA3 — peso 5.0%, rating Aguardar, PT R$25.0 |

##### 📈 Live snapshot (auto-gerado)

###### Preço
- **Drawdown 52w**: -23.10%
- **Drawdown 5y**: -23.10%
- **YTD**: -13.95%
- **YoY (1y)**: +27.05%
- **CAGR 3y**: +9.13%  |  **5y**: +1.11%  |  **10y**: n/a
- **Vol annual**: +37.33%
- **Sharpe 3y** (rf=4%): +0.14

###### Dividendos
- **DY 5y avg**: +1.72%
- **Div CAGR 5y**: +42.64%
- **Frequency**: annual
- **Streak** (sem cortes): 3 years

###### Valuation
- **P/E vs own avg**: n/a

##### 📈 Price history 1y

_Charts plugin requerido. Se não vês o gráfico: Settings → Community plugins → instalar **Charts** (phibr0)._

```chart
type: line
title: "VIVA3 — 1y close"
labels: ['2025-05-08', '2025-05-14', '2025-05-20', '2025-05-26', '2025-05-30', '2025-06-05', '2025-06-11', '2025-06-17', '2025-06-24', '2025-06-30', '2025-07-04', '2025-07-10', '2025-07-16', '2025-07-22', '2025-07-28', '2025-08-01', '2025-08-07', '2025-08-13', '2025-08-19', '2025-08-25', '2025-08-29', '2025-09-04', '2025-09-10', '2025-09-16', '2025-09-22', '2025-09-26', '2025-10-02', '2025-10-08', '2025-10-14', '2025-10-20', '2025-10-24', '2025-10-30', '2025-11-05', '2025-11-11', '2025-11-17', '2025-11-24', '2025-11-28', '2025-12-04', '2025-12-10', '2025-12-16', '2025-12-22', '2025-12-30', '2026-01-07', '2026-01-13', '2026-01-19', '2026-01-23', '2026-01-29', '2026-02-04', '2026-02-10', '2026-02-18', '2026-02-24', '2026-03-02', '2026-03-06', '2026-03-12', '2026-03-18', '2026-03-24', '2026-03-30', '2026-04-06', '2026-04-10', '2026-04-16', '2026-04-23', '2026-04-29', '2026-05-06']
series:
  - title: VIVA3
    data: [21.9, 23.06, 24.88, 25.58, 25.99, 25.23, 24.57, 25.12, 25.32, 27.03, 27.23, 26.2, 25.32, 24.79, 23.93, 25.72, 27.58, 27.08, 27.27, 27.78, 29.0, 28.95, 27.65, 28.72, 28.86, 28.5, 28.34, 27.4, 27.01, 29.1, 30.67, 30.87, 31.19, 31.93, 33.12, 32.71, 35.12, 36.0, 33.31, 34.52, 32.09, 33.24, 29.93, 29.5, 26.84, 28.35, 27.33, 28.6, 29.1, 30.4, 31.9, 30.83, 29.99, 26.37, 24.69, 26.42, 25.2, 26.03, 27.26, 27.5, 26.3, 24.95, 28.27]
width: 80%
beginAtZero: false
fill: false
tension: 0.3
```

##### 💰 Dividendos anuais (10y)

```chart
type: bar
title: "VIVA3 — dividend history"
labels: ['2019', '2020', '2021', '2022', '2023', '2024', '2025']
series:
  - title: Dividends
    data: [0.1693, 0.1058, 0.328, 0.0539, 0.3642, 0.3729, 1.3578]
width: 80%
beginAtZero: false
fill: false
tension: 0.3
```

##### 📊 Fundamentals trend

```chart
type: line
title: "P/E over time"
labels: ['2026-04-21', '2026-04-23', '2026-04-24', '2026-04-25', '2026-04-26', '2026-04-27', '2026-04-28', '2026-04-29', '2026-04-30', '2026-05-04', '2026-05-05', '2026-05-06', '2026-05-07']
series:
  - title: P/E
    data: [10.420454, 9.96212, 9.901514, 9.939163, 9.939163, 9.680608, 9.431818, 9.486691, 9.813687, 9.795454, 10.094696, 10.708333, 10.555133]
width: 80%
beginAtZero: false
fill: false
tension: 0.3
```

```chart
type: line
title: "ROE & DY %"
labels: ['2026-04-21', '2026-04-23', '2026-04-24', '2026-04-25', '2026-04-26', '2026-04-27', '2026-04-28', '2026-04-29', '2026-04-30', '2026-05-04', '2026-05-05', '2026-05-06', '2026-05-07']
series:
  - title: ROE %
    data: [22.75, 22.75, 22.75, 22.75, 22.75, 22.75, 22.75, 22.75, 22.75, 22.75, 22.75, 22.75, 22.75]
  - title: DY %
    data: [4.94, 2.65, 2.67, 2.67, 2.67, 2.74, 2.8, 2.8, 2.7, 2.7, 2.62, 2.47, 2.51]
width: 80%
beginAtZero: false
fill: false
tension: 0.3
```

---
*Gerado por obsidian_bridge — 2026-05-08 15:30 UTC*

#### — · IC Debate (synthetic)
_source: `cemetery\2026-05-14\ABSORBED-tickers\tickers\VIVA3_IC_DEBATE.md` (cemetery archive)_

#### 🏛️ Synthetic IC Debate — VIVA3

**Committee verdict**: **HOLD** (high confidence, 80% consensus)  
**Votes**: BUY=1 | HOLD=4 | AVOID=0  
**Avg conviction majority**: 5.0/10  
**Panel**: 5 personas (failed: 0)

##### 🗣️ Each persona's verdict

###### 🟡 Warren Buffett — **HOLD** (conv 5/10, size: small)

**Rationale**:
- ROE alto
- dívida controlada
- setor volátil

**Key risk**: volatilidade do setor de mineração afeta resultados

###### 🟢 Stan Druckenmiller — **BUY** (conv 8/10, size: medium)

**Rationale**:
- PE baixo e ROE alto indicam valor
- Dividend yield atraente
- Setor com atividades promissoras

**Key risk**: Volatilidade macroeconômica afetando o setor

###### 🟡 Nassim Taleb — **HOLD** (conv 5/10, size: small)

**Rationale**:
- P/E razoável
- ROE forte
- Nível de dívida controlado

**Key risk**: Volatilidade do setor mineral e exposição a eventos raros

###### 🟡 Seth Klarman — **HOLD** (conv 5/10, size: none)

**Rationale**:
- Valuation not compelling
- Industry risks present
- No significant margin of safety

**Key risk**: Volatility in commodity prices impacts profitability significantly

###### 🟡 Ray Dalio — **HOLD** (conv 5/10, size: medium)

**Rationale**:
- P/E baixo, ROE forte
- Dívida moderada
- Dividendos atraentes

**Key risk**: Volatilidade do mercado de commodities e exposição ao cenário macroeconômico brasileiro

##### 📊 Context provided

```
TICKER: BR:VIVA3

FUNDAMENTALS LATEST:
  pe: 9.418251
  pb: 1.9740198
  dy: 2.82%
  roe: 22.75%
  net_debt_ebitda: 1.0578497955001132
  intangible_pct_assets: 1.3%   (goodwill $0.0B + intangibles $0.1B)

RECENT MATERIAL NEWS (last 14d via Tavily):
  - Brazil's Vale posts $1.9 billion net profit in first quarter - Reuters [Tue, 28 Ap]
    REUTERS/Washington Alves Purchase Licensing Rights, opens new tab. SAO PAULO, April 28 (Reuters) - Brazilian ‌miner (VALE3.SA), opens new tab ​on ​Tuesday posted ⁠a $1.89 ​billion ​net profit for ​the
  - South Sudan declines to renew Oranto’s Block B3 exploration license - World Oil [Mon, 04 Ma]
    # South Sudan declines to renew Oranto’s Block B3 exploration license. (WO) - The Ministry of Petroleum of South Sudan has declined to renew the exploration and production sharing agreement (EPSA) hel
  - US company to carry out IPO of up to US$60.8 million to finance rare earth projects in Brazil - BNamericas [Mon, 04 Ma]
    # US company to carry out IPO of up to US$60.8 million to finance rare earth projects in Brazil. (REA) will carry out an initial public offering (IPO), with part of the proceeds allocated to rare eart
  - Russian diesel cargoes rerouted from Brazil as global prices surge - Reuters [Mon, 27 Ap]
    # Russian diesel cargoes rerouted from Brazil as global prices surge | Reuters. MOSCOW, April 27 (Reuters) - Two tankers carrying Russian ultra-low sulphur diesel (ULSD) ​were diverted mid-voyage from
```

---
*100% Ollama local (qwen2.5:14b-instruct-q4_K_M). Zero Claude tokens. 5 personas debated.*

##### 🎙️ Recent insights & mentions

_Auto-gerado · 2026-05-13 20:46 UTC · yt=0 · analyst=1 · themes=0_

###### 📰 Analyst reports (últimos 120d)

| Data | Fonte | Kind | Stance | PT | Claim |
|---|---|---|---|---:|---|
| 2026-04-24 | SUNO | rating | neutral | 25.00 | [Suno Dividendos] VIVA3 — peso 5.0%, rating Aguardar, PT R$25.0 |

## ⚙️ Refresh commands

```bash
ii panorama VIVA3 --write
ii deepdive VIVA3 --save-obsidian
ii verdict VIVA3 --narrate --write
ii fv VIVA3
python -m analytics.fair_value_forward --ticker VIVA3
```

---
_Gerado por `scripts/build_merged_hubs.py` em 2026-05-14. Run again to refresh._
