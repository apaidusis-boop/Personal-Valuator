---
type: ticker_hub
ticker: SANB11
market: br
sector: Banks
currency: BRL
bucket: research_pool
is_holding: false
generated: 2026-05-14
sources_merged: 4
tags: [hub, ticker, merged]
parent: "[[_TICKERS_INDEX]]"
---

# SANB11 — Santander BR

> **Hub mergeado**. Todo o conteúdo per-ticker do vault foi absorvido aqui (panorama, dossier, story, council, IC debate, variant, RI, filings, overnights, drips, wiki, reviews por persona, sessions). Ficheiros-fonte estão no `cemetery/2026-05-14/`.

`sector: Banks` · `market: BR` · `currency: BRL` · `bucket: research_pool` · `4 sources merged`

## 🎯 Hoje

- **Fundamentals** (2026-05-13): P/E 13.21 · P/B 0.81 · DY 8.5% · Dividend streak 17

## 📜 Histórico (conteúdo absorvido, ordem cronológica desc)

> Todas as fontes consolidadas (vault + JSON deepdives). Cada bloco mantém o título original e foi rebaixado 3 níveis (h1→h4) para encaixar.


### 2026

#### 2026-05-13 · Overnight scrape
_source: `cemetery\2026-05-14\ABSORBED-overnight-per-ticker\Overnight_2026-05-13\SANB11.md` (cemetery archive)_

#### SANB11 — Pilot Deep Dive (2026-05-12)

- **Market**: BR
- **Sector**: Banks
- **RI URLs scraped** (1):
  - https://www.santander.com/en/shareholders-and-investors
- **Pilot rationale**: tavily (watchlist)

##### Antes (estado da DB)

**Posição activa**: (nenhuma — watchlist ou holding sem qty)

- Total events na DB: **0**
- deep_fundamentals (rows anuais): **5**
- Última cotação DB: 2026-05-11 → close=27.829999923706055
- Último fundamentals snapshot: period_end=2026-05-11 · ROE=None · DY=0.08294779038190493 · P/E=13.602151
- Score (último run): score=0.0 · passes_screen=0

**Últimos 5 events em DB**:

_(zero events em DB)_

##### Agora (RI scrape live)

- Scrape: ❌ FALHOU — Traceback (most recent call last):
  File "C:\Users\paidu\investment-intelligence\fetchers\portal_playwright.py", line 233, in <module>
    main()
    ~~~~^^
  File "C:\Users\paidu\investment-intelligence\fetchers\portal_playwright.py", line 220, in main
    result = fetch(
        args.url,
    ...<5 lines>...
        headless=not args.no_headless,
    )
  File "C:\Users\paidu\investment-intellig


### (undated)

#### — · Panorama
_source: `cemetery\2026-05-14\ABSORBED-tickers\tickers\SANB11.md` (cemetery archive)_

#### SANB11 — Santander BR

#watchlist #br #banks

##### Links

- Sector: [[sectors/Banks|Banks]]
- Market: [[markets/BR|BR]]
- Peers: [[BBDC4]] · [[ABCB4]] · [[BBAS3]] · [[BPAC11]] · [[ITUB4]]

##### Snapshot

- **Preço**: R$28.42  (2026-05-07)    _-3.10% 1d_
- **Screen**: 0.0  ✗ fail
- **Altman Z**: n/a ()
- **Piotroski**: None/9
- **Div Safety**: 33.3/100 (RISK)

##### Fundamentals

- P/E: 13.890518 | P/B: 0.8481557 | DY: 8.12%
- ROE: None% | EPS: 2.046 | BVPS: 33.508
- Streak div: 17y | Aristocrat: None

##### Dividendos recentes

- 2026-04-22: R$0.5347
- 2026-01-21: R$0.5359
- 2026-01-05: R$0.1661
- 2025-10-22: R$0.5359
- 2025-07-18: R$0.5359

##### 🎙️ Recent insights & mentions

_Auto-gerado · 2026-05-13 20:46 UTC · yt=0 · analyst=1 · themes=0_

###### 📰 Analyst reports (últimos 120d)

| Data | Fonte | Kind | Stance | PT | Claim |
|---|---|---|---|---:|---|
| 2026-04-14 | XP | rating | bull | 44.00 | [XP Top Dividendos] SANB11 — peso 5.0%, Compra, PT R$44.0, setor Bancos |

##### 📈 Live snapshot (auto-gerado)

###### Preço
- **Drawdown 52w**: -24.21%
- **Drawdown 5y**: -35.15%
- **YTD**: -15.74%
- **YoY (1y)**: -0.46%
- **CAGR 3y**: -0.12%  |  **5y**: -5.40%  |  **10y**: +4.86%
- **Vol annual**: +28.24%
- **Sharpe 3y** (rf=4%): -0.16

###### Dividendos
- **DY 5y avg**: +6.96%
- **Div CAGR 5y**: -9.81%
- **Frequency**: quarterly
- **Streak** (sem cortes): 2 years

###### Valuation
- **P/E vs own avg**: n/a

##### 💰 Financials trend (annual)

| Period | Revenue | Net Income | Free Cash Flow |
|---|---|---|---|
| 2021-12-31 | n/a | n/a | n/a |
| 2022-12-31 | R$43.17B | R$14.29B | R$3.98B |
| 2023-12-31 | R$39.97B | R$9.45B | R$33.26B |
| 2024-12-31 | R$48.52B | R$13.37B | R$-23.74B |
| 2025-12-31 | R$46.68B | R$12.77B | R$1.34B |

##### 📈 Price history 1y

_Charts plugin requerido. Se não vês o gráfico: Settings → Community plugins → instalar **Charts** (phibr0)._

```chart
type: line
title: "SANB11 — 1y close"
labels: ['2025-05-08', '2025-05-14', '2025-05-20', '2025-05-26', '2025-05-30', '2025-06-05', '2025-06-11', '2025-06-17', '2025-06-24', '2025-06-30', '2025-07-04', '2025-07-10', '2025-07-16', '2025-07-22', '2025-07-28', '2025-08-01', '2025-08-07', '2025-08-13', '2025-08-19', '2025-08-25', '2025-08-29', '2025-09-04', '2025-09-10', '2025-09-16', '2025-09-22', '2025-09-26', '2025-10-02', '2025-10-08', '2025-10-14', '2025-10-20', '2025-10-24', '2025-10-30', '2025-11-05', '2025-11-11', '2025-11-17', '2025-11-24', '2025-11-28', '2025-12-04', '2025-12-10', '2025-12-16', '2025-12-22', '2025-12-30', '2026-01-07', '2026-01-13', '2026-01-19', '2026-01-23', '2026-01-29', '2026-02-04', '2026-02-10', '2026-02-18', '2026-02-24', '2026-03-02', '2026-03-06', '2026-03-12', '2026-03-18', '2026-03-24', '2026-03-30', '2026-04-06', '2026-04-10', '2026-04-16', '2026-04-23', '2026-04-29', '2026-05-06']
series:
  - title: SANB11
    data: [29.73, 30.41, 30.53, 30.2, 29.73, 28.94, 30.17, 30.15, 29.65, 29.69, 29.32, 27.96, 27.7, 26.4, 26.34, 25.82, 26.6, 27.0, 25.94, 27.31, 28.22, 28.19, 28.67, 28.87, 28.82, 29.24, 28.89, 27.94, 27.45, 28.9, 29.11, 30.61, 32.5, 33.41, 33.49, 33.09, 34.2, 34.7, 31.85, 31.76, 32.66, 34.06, 33.15, 33.49, 33.33, 35.74, 36.95, 34.97, 36.53, 35.14, 35.79, 33.52, 31.52, 30.58, 29.65, 29.97, 29.52, 31.06, 32.12, 31.45, 29.86, 28.64, 29.33]
width: 80%
beginAtZero: false
fill: false
tension: 0.3
```

##### 💰 Dividendos anuais (10y)

```chart
type: bar
title: "SANB11 — dividend history"
labels: ['2016', '2017', '2018', '2019', '2020', '2021', '2022', '2023', '2024', '2025', '2026']
series:
  - title: Dividends
    data: [0.9315, 1.6659, 1.765, 2.09, 2.8053, 2.8358, 2.2417, 1.6676, 1.6102, 1.8762, 1.2367]
width: 80%
beginAtZero: false
fill: false
tension: 0.3
```

##### 📊 Fundamentals trend

```chart
type: line
title: "P/E over time"
labels: ['2026-04-21', '2026-04-23', '2026-04-24', '2026-04-25', '2026-04-26', '2026-04-27', '2026-04-28', '2026-04-29', '2026-04-30', '2026-05-04', '2026-05-05', '2026-05-06', '2026-05-07']
series:
  - title: P/E
    data: [15.444771, 14.594331, 14.506354, 14.506354, 14.506354, 14.501467, 14.379276, 13.998045, 14.193549, 13.958944, 14.0615835, 14.335288, 13.890518]
width: 80%
beginAtZero: false
fill: false
tension: 0.3
```

```chart
type: line
title: "ROE & DY %"
labels: ['2026-04-21', '2026-04-23', '2026-04-24', '2026-04-25', '2026-04-26', '2026-04-27', '2026-04-28', '2026-04-29', '2026-04-30', '2026-05-04', '2026-05-05', '2026-05-06', '2026-05-07']
series:
  - title: ROE %
    data: [10.52, 10.52, 10.52, 10.52, 10.52, 10.52, 10.52, 10.52, 10.52, 10.52, 0, 0, 0]
  - title: DY %
    data: [6.88, 7.73, 7.78, 7.78, 7.78, 7.78, 7.85, 8.06, 7.95, 8.08, 8.02, 7.87, 8.12]
width: 80%
beginAtZero: false
fill: false
tension: 0.3
```

---
*Gerado por obsidian_bridge — 2026-05-08 15:30 UTC*

#### — · Deepdive (DOSSIE)
_source: `cemetery\2026-05-14\ABSORBED-tickers\tickers\SANB11_DOSSIE.md` (cemetery archive)_

#### 📑 SANB11 — Santander BR

> Generated **2026-04-26** by `ii dossier SANB11`. Cross-links: [[SANB11]] · [[SANB11]] · [[CONSTITUTION]]

##### TL;DR

<!-- TODO_CLAUDE_TLDR: 3 frases sobre SANB11 a partir das tabelas abaixo. Citar PE, DY, IC verdict, e o achado mais importante. -->

##### 1. Fundamentals snapshot

- **Período**: 2026-04-25
- **EPS**: 2.05  |  **BVPS**: 33.51
- **ROE**: 10.52%  |  **P/E**: 14.51  |  **P/B**: 0.89
- **DY**: 7.78%  |  **Streak div**: 17y  |  **Market cap**: R$ 140.10B
- **Last price**: BRL 29.68 (2026-04-24)  |  **YoY**: +5.7%

##### 2. Screen — BR Banks (CLAUDE.md)

| Critério | Threshold | Valor | OK? |
|---|---|---|---|
| P/E ≤ 10 | ≤ 10 | **14.51** | ❌ |
| P/B ≤ 1.5 | ≤ 1.5 | **0.89** | ✅ |
| DY ≥ 6% | ≥ 6% | **7.78%** | ✅ |
| ROE ≥ 12% | ≥ 12% | **10.52%** | ❌ |
| Streak div ≥ 5y | ≥ 5 | **17y** | ✅ |

→ **3/5 critérios** passam.

##### 3. Peer comparison

###### Fundamentals

| Métrica | SANB11 | ABCB4 | BBDC4 | ITUB4 |
|---|---|---|---|---|
| Market cap | R$ 140.10B | R$ 6.52B | R$ 210.57B | R$ 489.02B |
| P/E | 14.51 | 4.73 | 9.35 | 11.06 |
| P/B | 0.89 | 0.90 | 1.18 | 2.39 |
| ROE | 10.52% | 15.46% | 13.75% | 21.01% |
| DY | 7.78% | 10.30% | 7.56% | 7.68% |
| Streak div | 17y | 16y | 19y | 19y |
| YoY price | +5.7% | +21.0% | +48.9% | +31.5% |

###### BACEN regulatório (latest non-NULL)

| Métrica | SANB11 | ABCB4 | BBDC4 | ITUB4 |
|---|---|---|---|---|
| Período | n/a | 2025-09-30 | 2025-09-30 | 2025-09-30 |
| Basel | n/a | 16.71% | 15.85% | 16.40% |
| CET1 | n/a | 11.88% | 11.39% | 13.47% |
| NPL E-H | n/a | n/a | n/a | n/a |

##### 4. Synthetic IC

**🏛️ HOLD** (medium confidence, 60.0% consensus)

→ Detalhe: [[SANB11]]

##### 5. Thesis

**Core thesis (2026-04-25)**: A Santander BR (SANB11) é uma instituição financeira com um sólido histórico de dividendos, oferecendo um DY anualizado de 7.78% e operando com um P/B abaixo de 1.0, indicativo de desconto em relação ao patrimônio líquido, sugerindo margem de segurança para investidores.

**Key assumptions**:
1. A empresa mantém uma consistente política de dividendos por pelo menos mais 17 anos consecutivos
2. O P/B continuarão a refletir um desconto substancial em relação ao valor do patrimônio líquido, indicando continuidade da atual avaliação baixa
3. A rentabilidade sobre o capital próprio (ROE) se mantém acima de 10% para sustentar dividendos robustos
4. O mercado financeiro reconhece a solidez e consistência operacional do banco, pressionando o P/B para níveis mais eleva

→ Vault: [[SANB11]]

##### 6. Conviction breakdown

| Component | Score |
|---|---|
| **Composite** | **72** |
| Thesis health | 96 |
| IC consensus | 50 |
| Variant perception | 50 |
| Data coverage | 50 |
| Paper track | 90 |

##### Tutor

> Leitura métrica-por-métrica vs filosofia (CLAUDE.md screen). Cada link abre [[Glossary/_Index|Glossary]] para fórmula + contraméricas.

- **P/E = 14.51** → [[Glossary/PE|porquê isto importa?]]. Bancos BR têm spread alto e múltiplos comprimidos — target ≤ 10. **Actual 14.51** NÃO passa.
- **P/B = 0.89** → [[Glossary/PB|leitura completa]]. Bancos: P/B ≤ 1.5 = margem sobre equity. **0.89** OK.
- **DY = 7.78%** → [[Glossary/DY|leitura + contraméricas]]. BR DRIP: DY ≥ 6%. **7.78%** passa.
- **ROE = 10.52%** → [[Glossary/ROE|porque é a métrica chave Buffett]]. Bancos BR (Selic alta): target ≥ 12%. **10.52%** fraco.
- **Streak div = 17y** → [[Glossary/Dividend_Streak|porque importa]]. Target BR ≥ 5y; **passa**.
- **Basel = 16.71%** → [[Glossary/Basel_Ratio|capital regulatório]]. Tier **premium** (mín BCB ~10.5%; saudável ≥14%; premium ≥16%).
- **CET1 = 11.88%** → [[Glossary/CET1|capital high-quality]]. Tier **saudável** (≥11% médio peer BR; ≥13% leadership tipo ITUB4).

##### 7. Riscos identificados

<!-- TODO_CLAUDE_RISKS: 3-5 riscos prioritizados, baseados em IC + thesis + peer compare. Severidade 🟢🟡🔴. Cite trigger condition específica. -->

##### 8. Position sizing

**Status atual**: watchlist

<!-- TODO_CLAUDE_SIZING: guidance breve para entrada/aumento/redução. Considerar BR/US isolation, market cap, weight prudente, DRIP/cash deploy. -->

##### 9. Tracking triggers (auto-monitoring)

<!-- TODO_CLAUDE_TRIGGERS: 3-5 condições mensuráveis em SQL/data que indicariam re-avaliação. Ex: 'NPL > 4%', 'DY < 5.5%', 'thesis_health score < 60'. Citar tabela/coluna a monitorar. -->

##### 10. Compute trail

| Stage | Tool | Tokens Claude |
|---|---|---|
| Recon DB | sqlite3 | 0 |
| Vault read | filesystem | 0 |
| BACEN backfill | Olinda OData | 0 |
| Skeleton render | Python f-string | 0 |
| TODO_CLAUDE narrativa | Claude (subsequent edit) | ~600-1000 |

→ Re-run desta dossier (refresh): ~0.5s + 0 tokens (data layer só) ou ~600 tokens (re-fill narrativa).

---
*Generated by `ii dossier SANB11` on 2026-04-26. 100% in-house data. Fill TODO_CLAUDE_* markers para narrativa final.*

##### 🎙️ Recent insights & mentions

_Auto-gerado · 2026-05-13 20:46 UTC · yt=0 · analyst=1 · themes=0_

###### 📰 Analyst reports (últimos 120d)

| Data | Fonte | Kind | Stance | PT | Claim |
|---|---|---|---|---:|---|
| 2026-04-14 | XP | rating | bull | 44.00 | [XP Top Dividendos] SANB11 — peso 5.0%, Compra, PT R$44.0, setor Bancos |

#### — · IC Debate (synthetic)
_source: `cemetery\2026-05-14\ABSORBED-tickers\tickers\SANB11_IC_DEBATE.md` (cemetery archive)_

#### 🏛️ Synthetic IC Debate — SANB11

**Committee verdict**: **HOLD** (medium confidence, 60% consensus)  
**Votes**: BUY=2 | HOLD=3 | AVOID=0  
**Avg conviction majority**: 6.3/10  
**Panel**: 5 personas (failed: 0)

##### 🗣️ Each persona's verdict

###### 🟡 Warren Buffett — **HOLD** (conv 6/10, size: medium)

**Rationale**:
- Dividendos atrativos
- P/B abaixo de 1, sugerindo desconto
- ROE acima de 10%

**Key risk**: Santander pode enfrentar desafios regulatórios ou competitivos nos EUA com a fusão da Webster

###### 🟢 Stan Druckenmiller — **BUY** (conv 7/10, size: medium)

**Rationale**:
- Dividendos consistentes
- P/B abaixo de 1, indicando desconto
- ROE acima de 10%

**Key risk**: Suspensão temporária da recompra de ações pode afetar o valor

###### 🟡 Nassim Taleb — **HOLD** (conv 6/10, size: medium)

**Rationale**:
- Dividendos consistentes e sólido P/B
- ROE acima de 10%
- Histórico positivo

**Key risk**: Suspenção do programa de recompra pode indicar falta de capital para investimentos mais lucrativos

###### 🟢 Seth Klarman — **BUY** (conv 7/10, size: medium)

**Rationale**:
- P/B abaixo de 1, indicando desconto significativo
- Dividendos consistentes e robustos
- ROE acima de 10%

**Key risk**: Possível pressão sobre o P/B por novas aquisições ou fusões

###### 🟡 Ray Dalio — **HOLD** (conv 7/10, size: medium)

**Rationale**:
- Dividendos consistentes e sólida posição patrimonial
- P/B abaixo de 1 indica desconto significativo
- ROE acima de 10% sustenta dividendos

**Key risk**: Suspensão do programa de recompra de ações pode afetar confiança dos investidores

##### 📊 Context provided

```
TICKER: BR:SANB11

FUNDAMENTALS LATEST:
  pe: 14.506354
  pb: 0.88575864
  dy: 7.78%
  roe: 10.52%

VAULT THESIS:
**Core thesis (2026-04-25)**: A Santander BR (SANB11) é uma instituição financeira com um sólido histórico de dividendos, oferecendo um DY anualizado de 7.78% e operando com um P/B abaixo de 1.0, indicativo de desconto em relação ao patrimônio líquido, sugerindo margem de segurança para investidores.

**Key assumptions**:
1. A empresa mantém uma consistente política de dividendos por pelo menos mais 17 anos consecutivos
2. O P/B continuarão a refletir um desconto substancial em relação ao valor do patrimônio líquido, indicando continuidade da atual avaliação baixa
3. A rentabilidade sobre o capital próprio (ROE) se mantém acima de 10% para sustentar dividendos robustos
4. O mercado financeiro reconhece a solidez e consistência operacional do banco, pressionando o P/B para níveis mais eleva

RECENT MATERIAL NEWS (last 14d via Tavily):
  - Santander suspends buyback pending shareholder approval of Webster deal in US - Reuters [Thu, 23 Ap]
    REUTERS/Dado Ruvic/Illustration/File Photo Purchase Licensing Rights, opens new tab. MADRID, April 23 (Reuters) - Santander (SAN.MC), opens new tab will suspend its share ​buyback programme pending ap
  - ‘Digital with a human touch’: Inside Santander’s first global brand positioning - Marketing Week [Tue, 21 Ap]
    Santander is uniting its markets under a global brand positioning to build on the firm’s almost 200-year heritage. Santander is uniting its markets under a global brand positioning to build on the fir
  - Ecopetrol to acquire stake in Brazil’s Brava Energia, targets controlling interest - World Oil [Thu, 23 Ap]
    # Ecopetrol to acquire stake in Brazil’s Brava Energia, targets controlling interest. (WO) - **Ecopetrol** has agreed to acquire an initial equity stake in **Brava Energia** as part of a broader plan 
  - Blue Jays make trade, acquire 22-homer slugger from AL rivals - Yahoo Sports [Mon, 13 Ap]

```

---
*100% Ollama local (qwen2.5:14b-instruct-q4_K_M). Zero Claude tokens. 5 personas debated.*

##### 🎙️ Recent insights & mentions

_Auto-gerado · 2026-05-13 20:46 UTC · yt=0 · analyst=1 · themes=0_

###### 📰 Analyst reports (últimos 120d)

| Data | Fonte | Kind | Stance | PT | Claim |
|---|---|---|---|---:|---|
| 2026-04-14 | XP | rating | bull | 44.00 | [XP Top Dividendos] SANB11 — peso 5.0%, Compra, PT R$44.0, setor Bancos |

## ⚙️ Refresh commands

```bash
ii panorama SANB11 --write
ii deepdive SANB11 --save-obsidian
ii verdict SANB11 --narrate --write
ii fv SANB11
python -m analytics.fair_value_forward --ticker SANB11
```

---
_Gerado por `scripts/build_merged_hubs.py` em 2026-05-14. Run again to refresh._
