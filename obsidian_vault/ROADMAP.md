...
|---|---|---|
| **2026-05-08 noite (continuação)** | **Phase LL** Multi-Source Validation + Consensus | (1) `derive_fundamentals_from_filings.py` BR (CVM ITR/DFP → TTM) + bank YTD resolver. (2) `scoring/fair_value.py` prefere filings, yf fallback. (3) `analytics/data_confidence.py` 3-way voting (yf↔CVM↔Fundamentus). (4) `cvm_parser_bank.py` + minority-interest split. (5) Dual-class shares ON+PN para BBDC4/ITSA4. (6) Altman + Piotroski distress vetoes. (7) `fundamentus_fetcher.py` 3ª fonte BR. (8) `sec_xbrl_fetcher.py` US filings + restitch FY-9M para Q4 missing. (9) Outlier-aware median (VALE3 swap). (10) `fiis_com_br_fetcher.py` FII-specialized. (11) `config/macro_sector_fit.yaml` + macro overlay 4º gate. (12) `consensus_target.py` MVP + `/api/fair-trajectory` + `/api/consensus` endpoints. (13) Continuous scheduling em `daily_run.bat`. **14/33 holdings cross_validated**. PRIO3+TEN+VALE3 corretamente SELL via vetoes. Commits `40054ca`..`c09301a` (~13 commits). |
...
| 14 | **Dois Mundos split** (Pessoal & Comercial) | B1+B2 ✅ 2026-05-11 · B3 ~2-3d | `CONSTITUTION.md` → núcleo partilhado + [[CONSTITUTION_Pessoal]] / [[CONSTITUTION_Comercial]]; pasta `Comercial/` scaffold; blueprint movido. B3 = docs comerciais (competitive/pricing/compliance/GTM); B4 = report tier. Roadmap próprio: [[Comercial/00_Roadmap]] |

### Tier 3 — Aguarda dados / opportunistic

| Prioridade | Phase | Bloqueador | Done quando |
|---|---|---|---|
| 15 | **Phase GG** Capital Deployment Engine | ≥90d validated verdicts | Calibration curve estável; hit rate per-engine claro |
| 16 | **W.9** Remotion weekly video | depende W.5 ✅ + W.8 | Cron Sunday 20h + upload Drive + Telegram |
| 17 | **Phase JJ-AOW Tuning** | depende GG + EE estabilizar | Severity calibrada + Telegram inline buttons |
| — | **Dois Mundos B4** (report tier) + **B6** (infra multi-tenant) | B4: pós-validação verdicts (~Ago/2026) · B6: "go" comercial + ≥90d verdicts | ver [[Comercial/00_Roadmap]] |
