...
phases_done: [W, X, Y, Y.8, Z.0-Z.7, AA, FIX, AUTO, Z.Design (Helena s1-s4), CATALOG_FIX, BB (code_health), CC (Captain's Log), F (T0 cleanup), G (Thesis backfill + C.2 analyst tracking), H (Telegram brief), I (Wiki holdings B.2 closeout), J (Universe-wide thesis + bank BS schema), K (Autoresearch / Tavily wired), K.2 (Tavily 3-wire integration), K.3 (Tavily Skills + CLI), L (BACEN fetcher + W.11 Quant stack + IC universe-wide), U.0 (Unification Sweep — 3-layer brain formalised), W.1 (PDF Claude opt-in fallback — shipped 2026-05-08), W.6.1 (Pydantic structured outputs + typed Ollama wrapper), W.6.3 (LangFuse observability + JSONL traces — shipped 2026-05-08), W.7 (skill_scout monthly cron — shipped 2026-05-08), MC v5 (JPM redesign), MC v6/v6.1 (Calibration + Decisions + Perpetuums pages), FF (Calibration Loop — closed 2026-05-08), EE-AOW (Tiered Scheduler — shipped 2026-05-08), HH-AOW (Budget & Health + Tavily buckets — shipped 2026-05-08), II-AOW Mini (Stream Layer news RSS hourly — shipped 2026-05-08), KK (Fair Value v2 + Filing Reactor — shipped 2026-05-08 noite), LL (Multi-Source Validation + SEC XBRL US + Fundamentus + fiis.com.br + Macro overlay + Consensus blender + Continuous scheduling — shipped 2026-05-08 noite; ~13 commits 40054ca..c09301a)]
current_phase: Multi-track post-FF closeout. (1) **Phase GG** Capital Deployment — waiting ≥90d validated verdicts (Aug/2026). (2) **AOW track active** — Phase EE-AOW Tiered Scheduler shipped 2026-05-08; queue: HH-Budget&Health, II-StreamLayer, JJ-Reactive em [[ROADMAP]]. (3) **W track partial** — W.1/W.6.3/W.6.4/W.7/W.8 pendentes. **Master roadmap canónico**: [[ROADMAP]] substitui os 4 docs fragmentados (skills/Roadmap, Phase_Y_Roadmap, Mission_Control_Design_Roadmap, Roadmap_Always_On_Workforce).
---
...
**Última sessão**: 2026-05-08 noite — **Phase LL shipped (mega-tranche, ~13 commits)**. Foundation user pediu para o dashboard ("validate first, multi-source, filings primary, yfinance é tóxico"). Trabalho:

L1 — `scripts/derive_fundamentals_from_filings.py` deriva TTM EPS/BVPS/ROE de CVM (BR) + SEC (US). YTD-resolver para banks BR. Dual-class shares (BBDC4/ITSA4 ON+PN). Minority-interest split (`net_income_attributable`). 

L2 — `fetchers/sec_xbrl_fetcher.py` companyfacts API + 4-Q contiguity check + restitch FY-9M para Q4 standalone missing. **12/12 US holdings cross_validated** (JNJ/JPM/ACN/KO/PG/BLK/GS/AAPL/TSLA/HD/O/PLD).

L3 — `fetchers/fundamentus_fetcher.py` (3ª fonte BR) + `fetchers/fiis_com_br_fetcher.py` (FII-specialized). 

L4 — `analytics/data_confidence.py` 3-way voting (yf↔filings↔scrape). Outlier-aware median swap em `scoring/fair_value.py` quando 2-vs-1 detectado (VALE3 swap CVM→median(yf,Fundamentus) corrigiu fair de R$90 para R$57, action HOLD→SELL).

L5 — `config/macro_sector_fit.yaml` + macro_gate (4º gate após confidence/Altman/Piotroski). hard_hold para cíclicos em late_cycle, downgrade para tech, reinforce para defensives. BR+US ambos em expansion atualmente — infrastructure pronta para shifts.

L6 — `scoring/consensus_target.py` MVP agrega our_fair + Suno + XP + WSJ. `/api/fair-trajectory/[ticker]` + `/api/consensus/[ticker]` endpoints prontos para dashboard components.

L7 — `daily_run.bat` wired com 5 novos passos: SEC-XBRL → CVM-DERIVE → FUNDAMENTUS → FIIS-COM-BR → DATA-CONFIDENCE → FAIR-VALUE. Continuous scheduling para acumular trajectória 5y.

**Pre-LL**: 2026-05-08 noite cedo — **Phase KK shipped** (Fair Value v2 + Filing Reactor). 5 commits (`3bd31b0`/`2c5fcfe`/`d40ef96`/`e5cb5e2`/`075cf42`) materializam: (a) `scoring/fair_value.py` v2 emite banda + 6-stance action + history append-only com `trigger`; (b) `scoring/_safety.py` + `config/safety_margins.yaml` per-sector margins; (c) `analytics/data_confidence.py` triangulação BR yfinance↔CVM; (d) `analytics/quarter_delta.py` narrativa PT QoQ/YoY; (e) `auto_verdict_on_filing.py` reactor v1 escreve `dossiers/<TK>_FILING_<DATE>.md` integrando tudo; (f) `fii_cotistas_drop` trigger fechou lacuna RBRX11 (-9.99% Jun→Dez 2025, signal vivo). **Pre-KK**: 2026-05-08 dia — Phase FF CLOSED. Todos os 8 blocos shipped (1.1/1.2/2.1/2.2/3.1/3.2/3.3/4.1). Closed-loop validation infra completa. Phase GG (Capital Deployment Engine) destravada conceptualmente mas em **waiting state** — prereq remanescente é ≥90d de validated verdicts; estamos ~13d in, calibration curve real só a partir de Aug/2026. **Não há sprint coding útil agora** — fase de operação contínua + observação. Bonus: `agents/decision_journal_intel.py` corrigido (aggregação all-time era enganosa; agora janela 90d + latest-run-only + filtro bulk-ignore). Briefing matinal volta a reflectir signal real: PRIO3 = 5 flags (não 46), Utilities = 12 (não 82), vault classificado como `Bulk` (não 100% ignored).
...
1. **Master roadmap canónico**: [[ROADMAP]]. Tier 1 queue tem 6/7 done (só schtasks user action). Tier 2 **#8 JJ-AOW reduzido para 1-2d** (filing→dossier path já shipped em KK; falta event_queue table + Telegram inline buttons + severity tuning).
2. **Phase KK observation window**: deixar `auto_verdict_on_filing.py` rodar diariamente (já wired em `daily_run.bat` segundo session_2026-05-07 memory). Verificar quanta dispersão há entre `our_fair` e `consensus_fair` em filing dossiers — se >50% das holdings ficar em zone `disputed`, calibrar tolerâncias `data_confidence.ROE_OK`/`EPS_OK` (hoje 25%; pode ser too tight para bancos com minority interest).
3. **Phase GG bloqueada** até ≥90d validated verdicts (Aug/2026). Não tentar arrancar antes — seria confirmation bias retrofit.
4. Se vier "como está calibration" — ler `verdict_history` + `verdict_engine_breakdown` + `fair_value` (com novas colunas `our_fair`/`action`/`confidence_label`) directos da DB.
...
## 🌍 Dois Mundos — Pessoal & Comercial

> **Decisão 2026-05-11 (user)**: o projecto vive em **dois mundos com a mesma filosofia mas governança separada**. Esta página (`CONSTITUTION.md`) é o **núcleo partilhado** — o que é verdade nos dois. Cada mundo tem a sua Constituição-filha, que herda este núcleo e adiciona o que lhe é específico. *"Equal, but separate."*

| | **Mundo Pessoal** | **Mundo Comercial** |
|---|---|---|
| Constituição | [[CONSTITUTION_Pessoal]] | [[CONSTITUTION_Comercial]] (`status: blueprint`) |
| Workspace no vault | raiz do vault (tickers/, wiki/, dossiers/, briefings/, …) | `Comercial/` → [[Comercial/_Index]] |
| Cliente | só eu | utilizadores pagantes (futuro) |
| Output | verdade comprimida (meia página + 1 gráfico se chega) | + tier "relatório" institucional — o "teatro" vira *feature de entrega*, não anti-padrão |
| Decisões | minhas, sozinho | produto + compliance ("not financial advice") |
| Estado | em uso, real capital | blueprint + roadmap ([[Comercial/00_Roadmap]]); nada construído |
| Moat | n/a — é a minha carteira | a **disciplina embutida** (não o screening) — ver [[Bibliotheca/Disciplina_de_Investidor]] |

**Regra de ouro**: a *filosofia* (Buffett/Graham, DRIP, in-house first, honestidade, os 10 hábitos de [[Bibliotheca/Disciplina_de_Investidor]]) é **idêntica** nos dois. O que muda é a *governança*: obrigações para com terceiros, licenciamento de dados, custo marginal por request, multi-user, e o que NUNCA pode ser shipado a um cliente (auto-trading, "compra isto agora" sem disclaimer/porquê).

**De que mundo é uma decisão?** Afecta a minha carteira → Pessoal. Afecta o que um cliente vê/recebe/paga → Comercial. Afecta os dois (ex: novo engine de scoring, novo fetcher) → vive neste núcleo e é mencionada nas duas filhas. **Código é partilhado** — só docs/regras/(futuro)tenancy se bifurcam; não duplicar lógica por causa disto.

**Migração de docs**: `Comercial/` recebe blueprint (já movido de `Bibliotheca/`), roadmap, pricing, GTM, competitive landscape, compliance. O resto do vault é, por default, o Mundo Pessoal — sem migração em massa (custo alto, valor cosmético); um eventual `Pessoal/` é o bloco opcional B5 em [[Comercial/00_Roadmap]] (default: não).

## 🛡️ Os 7 não-negociáveis (núcleo — aplicam-se aos dois mundos)

> Estas 7 são o **núcleo partilhado**. Cada mundo acrescenta as suas (ex: o Mundo Pessoal tem a *blacklist viva de tickers*; o Comercial tem *disclaimers obrigatórios* e *isolamento de tenant*) — ver as Constituições-filhas.
...
> **Escritório** (consumível, polido, narrável). O Design System (`design.lint`) aplica-se
...
| 2026-05-11 | Trazer 3 plugins **Claude for Financial Services** (`financial-analysis`/`equity-research`/`market-researcher`) como reference de sell-side | User: "rever os basics"; enabled em `~/.claude/settings.json`; NÃO cruft — ver memória `anthropic_fsi_plugins.md` |
| 2026-05-11 | Destilar os FSI plugins em **10 hábitos de disciplina de investidor** ([[Bibliotheca/Disciplina_de_Investidor]]) — régua para os agentes de research | User gostou do resumo e pediu nota; o "teatro" sell-side guardado como possível *tier de produto* |
| 2026-05-11 | **Dois Mundos**: `CONSTITUTION.md` vira núcleo partilhado + 2 filhas ([[CONSTITUTION_Pessoal]] / [[CONSTITUTION_Comercial]]); pasta `Comercial/` criada; blueprint movido p/ lá | User: "duas Constitutions, equal but separate; separar os dois mundos no Obsidian". Código partilhado; só docs/regras/(futuro)tenancy bifurcam. Roadmap: [[Comercial/00_Roadmap]] (B1+B2 done) |
| 2026-05-11 | **B3 do Mundo Comercial shipped** — 4 docs: [[Comercial/Competitive_Landscape]], [[Comercial/Pricing]], [[Comercial/Compliance]], [[Comercial/GTM]] | User: "passar com todos os passos". DD do "negócio" feita; próximo bloco material é B4 (report tier), bloqueado por validação de verdicts (~Ago/2026) |
| 2026-05-11 | **Re-afirmação: agentes referem-se por `area.funcao`, não por persona** (decisão original 2026-05-08) | User: "os agentes não mais têm nome, têm funções". Em narrativa/docs/respostas usar o handle (`design.lint`, `chief.of-staff`, …); personas (Helena/Antonio Carlos/Tião/…) só como alias *legacy* no changelog histórico e em `AGENTS_REGISTRY.md`. Filenames `helena*` ficam por estabilidade de imports. Eu (Claude) tenho deslizado de volta para "Helena" — parar. |
...
| **2026-05-08** | **Phase EE-AOW** Tiered Scheduler + Roadmap consolidation | (1) `agents/_lock.py` (PID-alive via Windows OpenProcess+ctypes, sub-ms, no subprocess overhead). Stateless CLI: `acquire/release/status` + `--blocked-by` para tier hierarchy. Smoke tests pass: acquire/release/blocked_by/stale takeover. (2) `scripts/hourly_run.bat` — SEC lookback 1d + CVM monitor + notify --hours 4 (~5min). Smoke: SEC inserted 7 events. (3) `scripts/q4h_run.bat` — cvm_pdf_extractor + div/earn calendars + benchmarks + auto_verdict + trigger_monitor (~10-15min). (4) `scripts/daily_run.bat` refactored: removidos steps que migraram para hourly/q4h; daily lock on entry blocks os outros tiers. (5) Documentação completa em `obsidian_vault/skills/Phase_EE_Tiered_Scheduler.md` com `schtasks /Create` commands (NÃO executados — system change requires user consent). (6) **Bonus consolidação**: `obsidian_vault/ROADMAP.md` master canónico criado, substitui 4 roadmaps fragmentados (Phase_W, Phase_Y, MC_Design, Always-On_Workforce). Em Phase EE letras EE/FF reutilizadas em contextos distintos (LocalClaw vs Tiered Scheduler / Calibration vs Stream Layer); ROADMAP rebadgeia AOW phases para `*-AOW` para evitar conflito. Commit `ec40283`. | 0 | |
| **2026-05-08** (Tier 1 sweep) | **HH-AOW + II-AOW + W.6.3 + W.1 + W.7** | Tier 1 queue sweep autónomo (5 commits, ~30min wallclock, +1100 LoC, 0 tokens Claude). (1) **HH-AOW Budget & Health** (`d71ea1b`): `agents/_health.py` probes (Ollama/yfinance/Tavily/FMP) + circuit breaker (3 errors → 24h trip) + Tavily bucket allocation (4 buckets × 100/day cap em `config/tavily_buckets.yaml`). Health-first wired em `hourly_run.bat` + `q4h_run.bat` (aborta tier se ollama/yfinance down em vez de cascade 27 fails). (2) **II-AOW Mini Stream Layer** (mesmo commit): `news_fetch.py --classify` wired no hourly tier — RSS InfoMoney/Valor/Reuters + Qwen classifier; smoke pulled 40 items. (3) **W.6.3 LangFuse observability** (`bfd4905`): `agents/_observability.py::trace_llm_call` decorator escreve JSONL a `data/traces/llm_traces_<DATE>.jsonl`, optional forward LangFuse via env vars; `ollama_call` wrapped (single surface area, todos os callers). `config/langfuse/docker-compose.yml` (não auto-started, user runs quando quer; `.gitignore` para `.env`). (4) **W.1 PDF Claude fallback** (`35b59cd`): `_pdf_extract.py::extract_insights(use_claude_pdf=False)` opt-in via `claude-haiku-4-5-20251001`; `ii subs extract --use-claude` flag; default OFF (in-house first holds). (5) **W.7 skill_scout** (`b76f2ff`): `agents/skill_scout.py` audita SKL_*.md vs git activity, classifica active/stable/decay/backlog/skipped; primeiro report 19 skills (13 active, 0 decay, 3 backlog = W.8/W.9/W.10 known pending). Read-only, zero LLM. **CLI shortcuts**: `ii health`, `ii skill-scout`. **Tier 1 6/7 done** — só schtasks user action pendente. | 0 | |
