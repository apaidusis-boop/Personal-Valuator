...
phases_done: [W, X, Y, Y.8, Z.0-Z.7, AA, FIX, AUTO, Z.Design (Helena s1-s4), CATALOG_FIX, BB (code_health), CC (Captain's Log), F (T0 cleanup), G (Thesis backfill + C.2 analyst tracking), H (Telegram brief), I (Wiki holdings B.2 closeout), J (Universe-wide thesis + bank BS schema), K (Autoresearch / Tavily wired), K.2 (Tavily 3-wire integration), K.3 (Tavily Skills + CLI), L (BACEN fetcher + W.11 Quant stack + IC universe-wide), U.0 (Unification Sweep — 3-layer brain formalised), W.1 (PDF Claude opt-in fallback — shipped 2026-05-08), W.6.1 (Pydantic structured outputs + typed Ollama wrapper), W.6.3 (LangFuse observability + JSONL traces — shipped 2026-05-08), W.7 (skill_scout monthly cron — shipped 2026-05-08), MC v5 (JPM redesign), MC v6/v6.1 (Calibration + Decisions + Perpetuums pages), FF (Calibration Loop — closed 2026-05-08), EE-AOW (Tiered Scheduler — shipped 2026-05-08, distinct from EE-LocalClaw bundle of 2026-04-29), HH-AOW (Budget & Health + Tavily buckets — shipped 2026-05-08), II-AOW Mini (Stream Layer news RSS hourly — shipped 2026-05-08)]
...
1. **Master roadmap canónico**: [[ROADMAP]]. Tier 1 queue tem 7 itens ordenados por valor/esforço.
2. **AOW track active**: Phase EE-AOW shipped 2026-05-08. Próximo: HH-Budget&Health (1-2d) ou II-StreamLayer mini news RSS (1d). Pending user action: registar `schtasks /Create` para hourly+q4h (commands em [[skills/Phase_EE_Tiered_Scheduler]]).
3. **Phase GG bloqueada** até ≥90d validated verdicts (Aug/2026). Não tentar arrancar antes — seria confirmation bias retrofit.
4. Se vier "como está calibration" — ler `verdict_history` + `verdict_engine_breakdown` directos da DB. Window real abre depois de 30d.
