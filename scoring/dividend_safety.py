...
# US bank ticker whitelist (Financials sector + bank-like name) so JPM/BAC etc
# get the bank treatment. Mirrors scoring/engine.py and scoring/fair_value.py.
_US_BANK_TICKERS_DS = frozenset({
    "JPM", "BAC", "WFC", "C", "GS", "MS", "USB", "PNC", "TFC", "BK", "NTRS",
    "STT", "CFG", "RF", "KEY", "FITB", "MTB", "HBAN", "CMA", "CBSH", "WAL",
    "ZION", "COF", "DFS", "AXP",
})


def _is_bank(conn: sqlite3.Connection, ticker: str) -> bool:
    """Bank detection. Banks fund via deposits (not debt); net_debt_ebitda
    doesn't apply. Phase 2026-05-09: BBDC4/ITUB4 were getting RISK verdict
    from the leverage component alone, even though everything else passed."""
    if (ticker or "").upper() in _US_BANK_TICKERS_DS:
        return True
    r = conn.execute("SELECT sector FROM companies WHERE ticker=?", (ticker,)).fetchone()
    if not r or not r[0]:
        return False
    s = r[0].strip().lower()
    return s in ("banks", "banco", "bancos") or "bank" in s


def _is_holding(conn: sqlite3.Connection, ticker: str) -> bool:
    """Holding company detection. ITSA4/BN/BRK-B etc. Consolidated debt
    aggregates underlying-company debt and EBITDA is dividends received —
    leverage ratio is misleading. Treat as n/a, re-normalize weights."""
    r = conn.execute("SELECT sector FROM companies WHERE ticker=?", (ticker,)).fetchone()
    if not r or not r[0]:
        return False
    return r[0].strip().lower() == "holding"


def _is_fii(conn: sqlite3.Connection, ticker: str) -> bool:
    """BR FII detection. FIIs funded via cotas, not debt; same n/a treatment."""
    r = conn.execute("SELECT sector FROM companies WHERE ticker=?", (ticker,)).fetchone()
    if not r or not r[0]:
        return False
    s = r[0].strip().lower()
...
def _score_roe(roe: float | None, *, is_reit: bool = False) -> Component:
    if roe is None:
    ...
    if is_reit:
        # REITs run structurally low ROE (huge real-estate book vs. net income).
        # The generic 15%/10%/5% thresholds always score them RISK regardless
        # of dividend quality (O at 2.7% ROE penalized despite 33y consecutive
        # monthly increases). Two-tier approach:
        #   ≥5%: REIT-specific thresholds (some REITs do hit 5-8% — generally
        #        operationally efficient triple-net or specialised property)
        #   <5%: ROE simply doesn't apply — return n/a, re-normalize weights
        #        across the other 3 components (payout/streak/leverage).
        # Component name suffixed `_reit` so dossiers know this branch ran.
        if r >= 8:   return Component("roe_level_reit", 20, r/100, 20, "SAFE")
        if r >= 5:   return Component("roe_level_reit", 20, r/100, 15, "OK")
        # Below 5% in REIT context: drop the component entirely
        return Component("roe_level_reit", 20, r/100, None, "n/a")
...
def _score_leverage(net_debt_ebitda: float | None, *,
                    is_reit: bool = False,
                    is_bank: bool = False,
                    is_holding: bool = False,
                    is_fii: bool = False) -> Component:
    # Banks/holdings/FIIs: net_debt_ebitda doesn't have operational meaning.
    # Mark n/a so the component drops out and weights renormalize over
    # payout/roe/streak. ITSA4 was scoring RISK with 4.03x — but ITSA4's
    # "EBITDA" is dividends received from underlying ITUB4 etc; the metric
    # is misleading for a holding structure.
    if is_bank or is_holding or is_fii:
        kind = "bank" if is_bank else ("holding" if is_holding else "fii")
        return Component(f"net_debt_ebitda_{kind}_skip", 20, net_debt_ebitda, None, "n/a")
...
def compute(ticker: str, market: str | None = None) -> SafetyScore | None:
    ...
    with sqlite3.connect(_db(mkt)) as conn:
        ...
        is_bank = _is_bank(conn, ticker)
        is_holding = _is_holding(conn, ticker)
        is_fii = _is_fii(conn, ticker)

    if not fund and div_ttm == 0:
        return SafetyScore(ticker, mkt, 0.0, "N/A")

    # ROE fallback: when fundamentals.roe is null but eps + bvps exist, derive
    # roe = eps / bvps (mathematically equivalent to net_income / equity).
    # Catches BBDC4 (BR fetcher leaves roe null for banks) and similar gaps.
    roe = fund.get("roe")
    if roe is None and fund.get("eps") and fund.get("bvps") and fund["bvps"] != 0:
        roe = fund["eps"] / fund["bvps"]

    comps = [
        _score_payout(div_ttm, fund.get("eps"), ffo=fund.get("ffo"), is_reit=is_reit),
        _score_roe(roe, is_reit=is_reit),
        _score_streak(fund.get("streak"), fund.get("aristocrat")),
        _score_leverage(fund.get("net_debt_ebitda"),
                        is_reit=is_reit, is_bank=is_bank,
                        is_holding=is_holding, is_fii=is_fii),
    ]
