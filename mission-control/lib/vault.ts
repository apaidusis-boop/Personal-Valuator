...
const TICKERS_DIR = path.join(VAULT_DIR, "tickers");
...
// ── W7 · Synthetic-IC debate quote extractor ────────────────────────
//
// Reads obsidian_vault/tickers/{TK}_IC_DEBATE.md and pulls one quote
// from a persona that voted with the committee verdict (or the
// highest-conviction one if no match). Returns null if the file is
...
const PERSONA_HEADING_RE = /^###\s+[^\w]*\s*([A-Z][\w\s.\-']+?)\s+—\s+\*\*(BUY|HOLD|AVOID|MIXED)\*\*/i;

export function getICDebateQuote(ticker: string): ICQuote | null {
  const file = path.join(TICKERS_DIR, `${ticker.toUpperCase()}_IC_DEBATE.md`);
  ...
  const committee_verdict = String(data.committee_verdict || "MIXED").toUpperCase();
  const date = String(data.date || "");

  // Walk lines, find persona blocks + their first rationale bullet
  const lines = parsed.content.split("\n");
  type Block = { persona: string; verdict: string; rationale: string | null };
  const blocks: Block[] = [];
  let cur: Block | null = null;
  let inRationale = false;

  for (const line of lines) {
    const m = line.match(PERSONA_HEADING_RE);
    if (m) {
      if (cur) blocks.push(cur);
      cur = { persona: m[1].trim(), verdict: m[2].toUpperCase(), rationale: null };
      inRationale = false;
      continue;
    }
    if (!cur) continue;
    ...
    if (inRationale) {
      ...
      if (bullet && cur.rationale === null) {
        cur.rationale = bullet[1].trim();
      } else if (line.trim() === "") {
        // blank line ends rationale block
        if (cur.rationale !== null) inRationale = false;
  ...
  // Prefer a block whose verdict matches the committee verdict
  const matching = blocks.find((b) => b.verdict === committee_verdict && b.rationale);
  const fallback = blocks.find((b) => !!b.rationale);
  const chosen = matching || fallback;
  if (!chosen || !chosen.rationale) return null;

  // Capitalize first letter, ensure terminal punctuation
  let text = chosen.rationale;
