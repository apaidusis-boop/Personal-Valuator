  ...
  return (
    <div className="card p-5">
      <div className="flex items-center gap-2 mb-2">
        {icon && <span className="type-mono-sm text-[var(--text-tertiary)]">{icon}</span>}
        <h3 className="type-h3">{label}</h3>
      </div>
      <div className={"tabular " + (compact ? "type-h2" : "type-display")}>
        {value}
      </div>
      {(caption || delta) && (
...
function DeltaBadge({
  ...
  suffix = "%",
  format = "percent",
}: { value: number; suffix?: string; format?: "percent" | "absolute" }) {
  ...
  const formatted =
    format === "percent" ? Math.abs(value).toFixed(1) + suffix : Math.abs(value).toFixed(2);
  return (
