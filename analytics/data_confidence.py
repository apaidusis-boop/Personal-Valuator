...
def evaluate(ticker: str, market: str) -> dict:
    ...
    if market != "br":
        out.update({"label": "single_source", "score": None,
                    "detail": {"reason": "us_no_secondary_source",
                               "todo": "wire SEC EDGAR XBRL or finnhub when budget allows"}})
        return out

    db = DB_BR
    with sqlite3.connect(db) as c:
        yf = _yf_snapshot(c, ticker)
        cvm = _cvm_ttm(c, ticker)
    ...
    if cvm["n_quarters"] < 4:
        out.update({"label": "single_source", "score": 0.5,
                    "detail": {"reason": "cvm_insufficient_quarters",
