...
# Each persona is bound to a model family so the IC includes at least one
# member that doesn't share a latent space with the others:
#   Buffett/Klarman → Qwen 32B (slow, careful)
#   Druck/Dalio     → Qwen 14B (fast, broad)
#   Taleb           → Mistral 22B (different lab/training, restores diversity)
#
# Taleb was originally on `gemma4:31b` but that model is functionally broken
# on this Ollama 0.23.2 install — emits empty strings or `"own own own..."`
# degeneration in format=json mode. See:
#   obsidian_vault/Postmortem_2026-05-09_synthetic_ic.md
#   obsidian_vault/Postmortem_2026-05-09_synthetic_ic_followup.md
#
...
MODEL_TALEB    = "mistral-small:22b"  # was: gemma4:31b (broken), then qwen14b (no diversity)
        ...
        for k in ("pe", "pb", "dy", "roe", "net_debt_ebitda", "market_cap_usd", "current_ratio", "beta_levered"):
            ...
            if v is not None:
                if "margin" in k or k in ("dy", "roe"):
                    ...
                    lines.append(f"  {k}: {v}")
        # Intangible context (Phase 2026-05-09): surface goodwill + intangibles
        # so personas can reason about brand equity that's off balance sheet.
        # Critical for KO (P/B 10), PG (50% intangibles), JNJ (50%), where the
        # canonical Buffett ceiling formula `min(EPS×20, BVPS×3)` punishes
        # them for having strong brands not captured in book value.
        ipa = f.get("intangible_pct_assets")
        if ipa is not None:
            gw = f.get("goodwill") or 0
            oi = f.get("other_intangibles") or 0
            lines.append(f"  intangible_pct_assets: {ipa*100:.1f}%   "
                        f"(goodwill ${gw/1e9:.1f}B + intangibles ${oi/1e9:.1f}B)")
            if ipa >= 0.40:
                lines.append("  ⚠ HIGH intangibles — book value understates "
                            "brand/franchise economic value (P/B + Buffett ceiling unreliable)")
